...
class OpenFlowSwitchMixin(object):
    ...
    ##################################### lamtv10 start edit 1 here ###################################
    def is_exception_flow(self,in_port,table_id,priority):
        list_exception_flows=cfg.CONF.OVS.in_port_tableid_priority
        for flow in list_exception_flows:
            bl1=flow.find(':')
            bl2=flow[bl1+1:].find(':')+bl1+1
            f_in_port=flow[0:bl1]
            f_table_id=flow[bl1+1:bl2]
            f_priority=flow[bl2+1:]
            if in_port==f_in_port and table_id==f_table_id and ( priority==f_priority or f_priority=="")    :
                return True 
        return False
    ##################################### lamtv10 end edit 1 here ###################################
	...
	LOG.warning("Deleting flow with cookie 0x%(cookie)x table_id: %s priority %s match: %s",{'cookie': cookie}, str(table_id), str(cookie), str(match))
    ...
    def dump_flows(self, table_id=None):
        ...
        return flows
#------------------------------------------------------------------------------------------------------
    ##################################### lamtv10 start edit 2 here ###################################

    def cleanup_flows(self):
        
        LOG.info("Reserved cookies for %s: %s", self.br_name, self.reserved_cookies)
        for f in self.dump_flows():
            if f.cookie not in  self.reserved_cookies:
                if 'in_port' not in f.match:
                    #LOG.warning("Deleting flow with cookie 0x%(cookie)x table_id: %s priority %s match: %s",{'cookie': f.cookie}, str(f.table_id), str(f.cookie), str(f.match))
                    self.uninstall_flows(table_id=f.table_id,priority=f.priority,cookie=f.cookie, cookie_mask=ovs_lib.UINT64_BITMASK, match=f.match)
		    LOG.info("FLOW_NOT_Exception: %s",f)

                elif not self.is_exception_flow(str(f.match['in_port']),str(f.table_id),str(f.priority)):
                    #LOG.warning("Deleting flow with cookie 0x%(cookie)x table_id: %s priority %s match: %s",{'cookie': f.cookie}, str(f.table_id), str(f.cookie), str(f.match))
                    self.uninstall_flows(table_id=f.table_id,priority=f.priority,cookie=f.cookie, cookie_mask=ovs_lib.UINT64_BITMASK, match=f.match)
               	    LOG.info("DELETE FLOW: %s",f)
	
                else:
                    LOG.info("FLOW_Exception: %s",f)

    ##################################### lamtv10 end edit 2 here ###################################
#----------------------------------------------------------------------------------------------------
