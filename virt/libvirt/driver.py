        ...
        try:
            ...
            if CONF.libvirt.virt_type != "parallels":
                ...
                if json_instance_numa_topology['nova_object.data']['cells'][0]["nova_object.data"]['cpu_policy']=='dedicated' :
                    try: 
                        new_xml_str = libvirt_migrate.get_updated_guest_xml_with_reset_cpu_pinned(guest, migrate_data, self._get_volume_config, instance , dest)
                    except Exception as error:
                        LOG.exception(" get_updated_guest_xml_with_reset_cpu_pinned  failed %s ",instance, error=error)
                else :
                    new_xml_str = libvirt_migrate.get_updated_guest_xml(
                        guest, migrate_data, self._get_volume_config)
            LOG.debug("LAMTV10_______: new_xml_str %s", new_xml_str)
...
################################################### lamtv10 start edit in here ###############################
    ...
    def post_live_migration_at_destination(self, context,
        ...
        # made it as expected.
        LOG.debug(" instance GET GUEST %s" ,  instance.uuid)
        LOG.debug(" lamtv10 recreate xml for libvirtd : %s",self._host.get_guest(instance).get_xml_desc())
