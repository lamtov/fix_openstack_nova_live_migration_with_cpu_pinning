...
def get_updated_guest_xml_with_reset_cpu_pinned(guest,migrate_data,get_volume_config,instance,dest):
    xml_doc = etree.fromstring(guest.get_xml_desc(dump_migratable=True))
    xml_doc = _update_graphics_xml(xml_doc, migrate_data)
    xml_doc = _update_serial_xml(xml_doc, migrate_data)
    xml_doc = _update_volume_xml(xml_doc, migrate_data, get_volume_config)
    xml_doc = _update_perf_events_xml(xml_doc, migrate_data)
    try:
        LOG.info("start reset_vcpu_pin  instance: %s dest : %s ", instance, dest )
        xml_doc = _update_cpu_pin_xml(xml_doc, instance, dest)
    except Exception as error:
        LOG.exception(" _update_cpu_pin_xml  failed %s ",instance, error=error)
    return etree.tostring(xml_doc, encoding='unicode')


def _update_cpu_pin_xml(xml_doc, instance, dest):
    size_vcpus_instance=instance.vcpus
    size_ram_instance=instance.memory_mb
    instance_numa_topology=  jsonutils.loads(instance.numa_topology._to_json())

    if instance_numa_topology.get('nova_object.data'):
        cpu_policy=instance_numa_topology['nova_object.data']['cells'][0]["nova_object.data"]['cpu_policy']
        if cpu_policy == 'dedicated':
            ...
            compute_numa_topology = jsonutils.loads(db_connection.execute(db.select([db_nova_compute_nodes.columns.numa_topology]).where(db_nova_compute_nodes.columns.hypervisor_hostname==dest)).fetchall()[0][0])
            is_need_change = True
            for numa_cell in compute_numa_topology["nova_object.data"]["cells"]:
                len_cpuset= len(numa_cell["nova_object.data"]["cpuset"])
                #LOG.error("lamtv10___ len_cpuset :  %s ",len_cpuset)
                numa_cell_pinned_cpus=numa_cell["nova_object.data"]["pinned_cpus"];
                len_pinned_cpus= len(numa_cell_pinned_cpus)
                #LOG.error("lamtv10___ len_pinned_cpus :  %s ",len_pinned_cpus)
                size_ram_numa_cell= numa_cell["nova_object.data"]["memory"]
                #LOG.error("lamtv10___ size_ram_numa_cell :  %s ",size_ram_numa_cell)
                used_ram_numa_cell= numa_cell["nova_object.data"]["memory_usage"]
                #LOG.error("lamtv10___ used_ram_numa_cell :  %s ",used_ram_numa_cell)
                if len_cpuset - len_pinned_cpus >= size_vcpus_instance and size_ram_numa_cell - used_ram_numa_cell >= size_ram_instance and is_need_change == True :
                    #LOG.error("lamtv10______________ compute_numa_topology: %s" , compute_numa_topology)
                    ...
                    LOG.debug("lamtv10______________ new_pinned_cpu: %s" , new_pinned_cpu)
                    vcpupins_xml_doc = xml_doc.findall('./cputune/vcpupin')
                    numa_emulatorpin_xml_doc=xml_doc.findall('./cputune/emulatorpin')
                    numa_memory_xml_doc = xml_doc.findall('./numatune/memory')
                    numa_memnode_xml_doc = xml_doc.findall('./numatune/memnode')
                    for i in range(0,len(vcpupins_xml_doc)):
                        LOG.debug("lamtv10______________ new_pinned_cpu: %s %s" ,i, new_pinned_cpu[i])
                        vcpupins_xml_doc[i].set('cpuset',str(new_pinned_cpu[i]))
                    for i in range(0,len(numa_emulatorpin_xml_doc)):
                        numa_emulatorpin_xml_doc[i].set('cpuset',str(','.join(str(vcpu) for vcpu in new_pinned_cpu)))
                    for i in range(0,len(numa_memory_xml_doc)):
                        numa_memory_xml_doc[i].set('nodeset',str(numa_node_id))
                    for i in range(0,len(numa_memnode_xml_doc)):
                        numa_memnode_xml_doc[i].set('nodeset',str(numa_node_id))
                    is_need_change = False


    #for vcpupin in xml_doc.findall('./cputune/vcpupin'):
    #    LOG.error("lamtv10______________ vcpupin: %s ", vcpupin.get('cpuset'))
    #for emulatorpin in xml_doc.findall('./cputune/emulatorpin'):
    #    LOG.error("lamtv10______________ emulatorpin: %s ", emulatorpin.get('cpuset'))
    #for memory in xml_doc.findall('./numatune/memory'):
    #    LOG.error("lamtv10______________ memory: %s ", memory.get('nodeset'))
    #for emulatorpin in xml_doc.findall('./numatune/memnode'):
    #    LOG.error("lamtv10______________ memnode: %s ", emulatorpin.get('nodeset'))
    return xml_doc
